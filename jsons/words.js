var wordsJson = {
	"lessons" : {
		"1": {
			"sections" : {
				"1": {
					"words" : [
						{
							"portuguese": "professor (m.)",
							"english": "teacher"
						},
						{
							"portuguese": "professora (f.)",
							"english": "teacher"
						},
						{
							"portuguese": "o/a/os/as",
							"english": "the"
						},
						{
							"portuguese": "e",
							"english": "and"
						},
						{
							"portuguese": "médico",
							"english": "doctor"
						},
						{
							"portuguese": "estudante (m./f.)",
							"english": "student (m./f.)"
						},
						{
							"portuguese": "escritório",
							"english": "office"
						},
						{
							"portuguese": "casa",
							"english": "house"
						},
						{
							"portuguese": "bem",
							"english": "well, fine"
						},
						{
							"portuguese": "bom (m.), boa (f.)",
							"english": "good"
						},
						{
							"portuguese": "pequeno",
							"english": "small"
						},
						{
							"portuguese": "muito",
							"english": "very"
						}
					],
					"beforeNotes": "Each unit begins with a lesson that focuses on words. The words will be used throughout the unit, so familiarize yourself with them. For advice on learning new vocabulary, consult the _Language learning tips_ section at the beginning of this program."
				},
				"2": {
					"words" : [
						{
							"portuguese": "o marido",
							"english": "the husband"
						},
						{
							"portuguese": "a mulher",
							"english": "the wife/woman"
						},
						{
							"portuguese": "a criança",
							"english": "the child (m./f.)"
						},
						{
							"portuguese": "os livros",
							"english": "the books"
						},
						{
							"portuguese": "ou",
							"english": "or"
						},
						{
							"portuguese": "o jornal",
							"english": "the newspaper"
						},
						{
							"portuguese": "a profissão",
							"english": "the occupation"
						},
						{
							"portuguese": "também",
							"english": "too, also"
						},
						{
							"portuguese": "o celular",
							"english": "the cell phone"
						},
						{
							"portuguese": "onde",
							"english": "where"
						},
						{
							"portuguese": "Tchau!",
							"english": "Bye! (infml.)"
						},
						{
							"portuguese": "Oi!",
							"english": "Hi! (infml.)"
						},
						{
							"portuguese": "Até logo!",
							"english": "Bye! (fml.)"
						},
						{
							"portuguese": "pouco",
							"english": "little"
						}
					]
				}
			}
		},
		"5": {
			"sections" : {
				"1": {
					"words" : [
						{
							"portuguese": "o pai, a mãe",
							"english": "father, mother"
						},
						{
							"portuguese": "o filho, a filha",
							"english": "son, daughter"
						},
						{
							"portuguese": "o primo, a prima",
							"english": "cousin"
						},
						{
							"portuguese": "o irmão, a irmã",
							"english": "brother, sister"
						},
						{
							"portuguese": "o avô, a avó",
							"english": "grandfather, grandmother"
						},
						{
							"portuguese": "o tio, a tia",
							"english": "uncle, aunt"
						},
						{
							"portuguese": "o sobrinho, a sobrinha",
							"english": "nephew, niece"
						},
						{
							"portuguese": "o marido",
							"english": "husband"
						},
						{
							"portuguese": "a mulher",
							"english": "wife, woman"
						},
						{
							"portuguese": "a gente",
							"english": "people, we (conversational)"
						},
						{
							"portuguese": "a pessoa, as pessoas",
							"english": "person, people"
						}
					],
					"beforeNotes": "Don't forget that *o* is _the_ (m.) and *a* is _the_ (f.). With these articles, you'll be able to tell the gender of nouns in the vocabulary lists.",
					"afterNotes": "*A Gente* literally means _the people_, but it's also used in informal conversation to mean _we_: *a gente é da Argentina* (_we are from Argentina_), *a gente é estudante* (_we are students_). Compare this to *a pessoa* (_person_) and *as pessoas* (_people, more than one individual person_). To talk about people, as in a group of people from a country, use *o povo*: *o povo brasileiro é feliz* (_Brazilians are a happy people_)."
				},
				"2": {
					"words" : [
						{
							"portuguese": "o quarto",
							"english": "bedroom"
						},
						{
							"portuguese": "o computador",
							"english": "computer"
						},
						{
							"portuguese": "o toca CD",
							"english": "CD player"
						},
						{
							"portuguese": "a televisão/a TV",
							"english": "television/TV"
						},
						{
							"portuguese": "o sofá",
							"english": "sofa"
						},
						{
							"portuguese": "a cadeira",
							"english": "chair"
						},
						{
							"portuguese": "a mesa",
							"english": "table"
						},
						{
							"portuguese": "a sala",
							"english": "living room"
						},
						{
							"portuguese": "a cozinha",
							"english": "kitchen"
						},
						{
							"portuguese": "o banheiro",
							"english": "bathroom"
						},
						{
							"portuguese": "a janela",
							"english": "window"
						},
						{
							"portuguese": "a porta",
							"english": "door"
						},
						{
							"portuguese": "a piscina",
							"english": "swimming pool"
						},
						{
							"portuguese": "Tudo bem?",
							"english": "How's it going?/How are you doing?"
						},
						{
							"portuguese": "Puxa!",
							"english": "Wow!/Gee!/Oh, no!"
						},
						{
							"portuguese": "Que pena!",
							"english": "What a pity/That's too bad!"
						}
					]
				}
			}
		},
		"9": {
			"sections" : {
				"1": {
					"words" : [
						{
							"portuguese": "a hora",
							"english": "hour"
						},
						{
							"portuguese": "a manhã",
							"english": "morning"
						},
						{
							"portuguese": "a tarde",
							"english": "afternoon"
						},
						{
							"portuguese": "a noite",
							"english": "night"
						},
						{
							"portuguese": "atrasado/a",
							"english": "late"
						},
						{
							"portuguese": "adiantado/a",
							"english": "early"
						},
						{
							"portuguese": "na hora",
							"english": "on time"
						},
						{
							"portuguese": "o relógio",
							"english": "clock/watch"
						},
						{
							"portuguese": "todo/s",
							"english": "every"
						},
						{
							"portuguese": "o minuto",
							"english": "minute"
						},
						{
							"portuguese": "o segundo",
							"english": "second"
						},
						{
							"portuguese": "quase",
							"english": "almost"
						},
						{
							"portuguese": "hoje",
							"english": "today"
						},
						{
							"portuguese": "a atividade",
							"english": "activity"
						},
						{
							"portuguese": "a aula",
							"english": "lesson/class"
						},
						{
							"portuguese": "a academia",
							"english": "gym"
						},
						{
							"portuguese": "o despertador",
							"english": "alarm clock"
						},
						{
							"portuguese": "o rádio-relógio",
							"english": "clock radio"
						}
					]
				},
				"2": {
					"words" : [
						{
							"portuguese": "despois",
							"english": "after"
						},
						{
							"portuguese": "antes",
							"english": "before"
						},
						{
							"portuguese": "o café da manhã",
							"english": "breakfast"
						},
						{
							"portuguese": "o chuveiro",
							"english": "shower"
						},
						{
							"portuguese": "o banho",
							"english": "bath"
						},
						{
							"portuguese": "a escova de dentes",
							"english": "toothbrush"
						},
						{
							"portuguese": "a pasta de dentes",
							"english": "toothpaste"
						},
						{
							"portuguese": "a escova de cabelo",
							"english": "hairbrush"
						},
						{
							"portuguese": "o sabonete",
							"english": "soap"
						},
						{
							"portuguese": "a garrafa de cerveja",
							"english": "beer bottle"
						},
						{
							"portuguese": "o frasco de xampu",
							"english": "shampoo bottle"
						},
						{
							"portuguese": "o aparelho de barbear",
							"english": "razor"
						},
						{
							"portuguese": "a xícara de café",
							"english": "coffee cup"
						},
						{
							"portuguese": "a colher de sopa",
							"english": "soup spoon"
						},
						{
							"portuguese": "o copo de vinho",
							"english": "wineglass"
						}
					]
				}
			}
		},
		"13": {
			"sections" : {
				"1": {
					"words" : [
						{
							"portuguese": "a saúde",
							"english": "health"
						},
						{
							"portuguese": "a cabeça",
							"english": "head"
						},
						{
							"portuguese": "a boca",
							"english": "mouth"
						},
						{
							"portuguese": "o olho",
							"english": "eye"
						},
						{
							"portuguese": "a orelha",
							"english": "ear"
						},
						{
							"portuguese": "o nariz",
							"english": "nose"
						},
						{
							"portuguese": "a garganta",
							"english": "throat"
						},
						{
							"portuguese": "o pescoço",
							"english": "neck"
						},
						{
							"portuguese": "a perna",
							"english": "leg"
						},
						{
							"portuguese": "o rosto",
							"english": "face"
						},
						{
							"portuguese": "o braço",
							"english": "arm"
						},
						{
							"portuguese": "o pé",
							"english": "foot"
						},
						{
							"portuguese": "a mão",
							"english": "hand"
						},
						{
							"portuguese": "o estômago",
							"english": "stomach"
						},
						{
							"portuguese": "o dente",
							"english": "tooth"
						},
						{
							"portuguese": "o dedo",
							"english": "finger/toe"
						},
						{
							"portuguese": "o coração",
							"english": "heart"
						},
						{
							"portuguese": "o pulmão",
							"english": "lung"
						},
						{
							"portuguese": "a sangue",
							"english": "blood"
						},
						{
							"portuguese": "o osso",
							"english": "bone"
						}
					]
				},
				"2": {
					"words" : [
						{
							"portuguese": "o remédio",
							"english": "medicine"
						},
						{
							"portuguese": "a vitamina",
							"english": "vitamin"
						},
						{
							"portuguese": "a consulta médica",
							"english": "doctor's appointment"
						},
						{
							"portuguese": "a receita médica",
							"english": "medical prescription"
						},
						{
							"portuguese": "o pronto socorro",
							"english": "emergency room"
						},
						{
							"portuguese": "em forma",
							"english": "fit, in shape"
						},
						{
							"portuguese": "a disposição",
							"english": "state of mind, condition, disposition"
						},
						{
							"portuguese": "cansado(a)",
							"english": "tired"
						},
						{
							"portuguese": "a injeção",
							"english": "injection"
						},
						{
							"portuguese": "a dor",
							"english": "ache, pain"
						},
						{
							"portuguese": "doente",
							"english": "sick, ill"
						},
						{
							"portuguese": "a infecção",
							"english": "infection"
						},
						{
							"portuguese": "a gripe",
							"english": "flu"
						},
						{
							"portuguese": "o resfriado",
							"english": "a cold"
						},
						{
							"portuguese": "a aspirina",
							"english": "aspirin"
						},
						{
							"portuguese": "o curativo Band-Aid",
							"english": "adhesive bandage Band-Aid"
						},
						{
							"portuguese": "o xarope",
							"english": "cough syrup"
						},
						{
							"portuguese": "o colírio",
							"english": "eye drops"
						}
					]
				}
			}
		}
	}
}