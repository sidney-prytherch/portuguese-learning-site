var sentencesJson = {
	"lessons" : {
		"3": {
			"sections" : {
				"1": {
					"sentences" : [
						{
							"portuguese": "Este é meu amigo Roberto.",
							"english": "This is my friend Roberto."
						},
						{
							"portuguese": "Eu sou americana.",
							"english": "I am American. (f.)"
						},
						{
							"portuguese": "Eu sou de Nova Iorque.",
							"english": "I'm from New York."
						},
						{
							"portuguese": "Você é do Rio?",
							"english": "Are you from Rio?"
						},
						{
							"portuguese": "Nós não somos do Rio.",
							"english": "We're not from Rio."
						},
						{
							"portuguese": "Eu nasci em Curitiba.",
							"english": "I was born in Curitiba."
						},
						{
							"portuguese": "De onde você é?",
							"english": "Where are you from?"
						},
						{
							"portuguese": "Eu sou casado.",
							"english": "I am married. (m.)"
						},
						{
							"portuguese": "Ele é solteiro.",
							"english": "He's single."
						},
						{
							"portuguese": "Elas são daqui.",
							"english": "They're from here. (f.)"
						}
					]
				},
				"2": {
					"sentences" : [
						{
							"portuguese": "Gostaria de apresentá-lo ao Sr. José Luís.",
							"english": "I'd like to introduce you to Mr. José Luís."
						},
						{
							"portuguese": "Ouvi falar que é muito grande.",
							"english": "I've head it's very big."
						},
						{
							"portuguese": "Muito prazer em conhecê-lo também.",
							"english": "It's a pleasure to meet you, too."
						},
						{
							"portuguese": "Nós somos de São Paulo.",
							"english": "We're from São Paulo. (m.)"
						},
						{
							"portuguese": "Elas são italianas.",
							"english": "They're Italian. (f.)"
						},
						{
							"portuguese": "Ele é nosso gerente de verdas.",
							"english": "He's our sales manager."
						},
						{
							"portuguese": "Como você se chama?",
							"english": "What's your name?"
						},
						{
							"portuguese": "Esta loja é grande.",
							"english": "This store is big."
						},
						{
							"portuguese": "Estas ruas são estreitas.",
							"english": "These streets are narrow."
						},
						{
							"portuguese": "Não conheço os Estados Unidos.",
							"english": "I don't know the United States."
						}
					]
				}
			}
		},
		"7": {
			"sections" : {
				"1": {
					"sentences" : [
						{
							"portuguese": "O meu passaporte é americano.",
							"english": "My passport is American."
						},
						{
							"portuguese": "A minha mulher é advogada.",
							"english": "My wife is a lawyer."
						},
						{
							"portuguese": "Os meus olhos são castanhos.",
							"english": "My eyes are brown."
						},
						{
							"portuguese": "As minhas primas são solteiras.",
							"english": "My cousins are single. (f.)"
						},
						{
							"portuguese": "Os meus pais são velhos.",
							"english": "My parents are old."
						},
						{
							"portuguese": "O meu país é moderno.",
							"english": "My country is modern."
						},
						{
							"portuguese": "Os seus documentos estão em ordem.",
							"english": "Your documents are in order."
						},
						{
							"portuguese": "Os nossos pratos são velhos",
							"english": "Our plates are old."
						},
						{
							"portuguese": "As nossas amigas são húngaras.",
							"english": "Our friends are Hungarian."
						},
						{
							"portuguese": "A sua colher está suja.",
							"english": "Your spoon is dirty."
						},
						{
							"portuguese": "As nossas facas estão limpas.",
							"english": "Our knives are clean."
						}
					]
				},
				"2": {
					"sentences" : [
						{
							"portuguese": "Os nossos maridos estão aqui.",
							"english": "Our husbands are here."
						},
						{
							"portuguese": "A minha prima é italiana.",
							"english": "My cousin is Itlatian."
						},
						{
							"portuguese": "A minha prima está em casa.",
							"english": "My cousin is at home."
						},
						{
							"portuguese": "O banheiro está ocupada.",
							"english": "The bathroom is occupied."
						},
						{
							"portuguese": "O meu quarto é pequeno, mas é confortável.",
							"english": "My room is small, but is it comfortable."
						},
						{
							"portuguese": "Os sofás são caros, mas são lindos.",
							"english": "The sofas are expensive, but they are nice."
						},
						{
							"portuguese": "Os seus olhos são castanhos.",
							"english": "Are your eyes brown?"
						},
						{
							"portuguese": "O meu carro está na minha casa.",
							"english": "My car is in my house."
						},
						{
							"portuguese": "O meu quarto não está limpo.",
							"english": "My room is not clean."
						},
						{
							"portuguese": "Os nossos quartos são ótimas.",
							"english": "Our rooms are great."
						}
					]
				}
			}
		},
		"11": {
			"sections" : {
				"1": {
					"sentences" : [
						{
							"portuguese": "Quando ele estuda?",
							"english": "When does he study?"
						},
						{
							"portuguese": "Ele estuda de noite.",
							"english": "He studies at night."
						},
						{
							"portuguese": "Onde vocês moram?",
							"english": "Where do you live?"
						},
						{
							"portuguese": "Nós moramos na Rua Pamplona, número 26.",
							"english": "We live at 26 Pamplona Street."
						},
						{
							"portuguese": "Por que ela estuda português?",
							"english": "Why does she study Portuguese?"
						},
						{
							"portuguese": "Ela estuda português porque gosta.",
							"english": "She studies Portuguese because she likes it."
						},
						{
							"portuguese": "O que vocês estudam?",
							"english": "What do you study?"
						},
						{
							"portuguese": "Estudamos português.",
							"english": "We study Portuguese."
						},
						{
							"portuguese": "Como elas viajam?",
							"english": "How do they travel?"
						},
						{
							"portuguese": "Elas viajam de trem.",
							"english": "They travel by train."
						},
						{
							"portuguese": "A professora está atrasada.",
							"english": "The teacher is late."
						},
						{
							"portuguese": "O avião está adiantado.",
							"english": "The plane is early."
						},
						{
							"portuguese": "O relógio está adiantado.",
							"english": "The clock is fast."
						},
						{
							"portuguese": "Meu relógio está atrasado.",
							"english": "My watch is slow."
						},
						{
							"portuguese": "Mariana adora festas.",
							"english": "Mariana loves parties."
						},
						{
							"portuguese": "Mariana ama Paulo.",
							"english": "Mariana loves Paulo."
						},
						{
							"portuguese": "Eu odeio trânsito.",
							"english": "I hate traffic."
						},
						{
							"portuguese": "Nós não supartamos o calor.",
							"english": "We can't stand the heat."
						}
					]
				},
				"2": {
					"sentences" : [
						{
							"portuguese": "Como é seu sobrenome?",
							"english": "What's your last name?"
						},
						{
							"portuguese": "Meu sobrenome é Smith.",
							"english": "My last name is Smith."
						},
						{
							"portuguese": "O que você faz de manhã",
							"english": "What do you do in the morning?"
						},
						{
							"portuguese": "Eu acordo às seis, tomo banho, depois tomo café da manhã, e estudo português.",
							"english": "I wake up at 6:00, take a shower, then I have breakfast, and I study Portuguese."
						},
						{
							"portuguese": "O que você faz no seu tempo livre?",
							"english": "What do you do in your free time?"
						},
						{
							"portuguese": "Eu faço muitas coisas interessantes no meu tempo livre.",
							"english": "I do lots of interesting things in my free time."
						},
						{
							"portuguese": "Eles fazem muitas perguntas sobre o meu dia a dia.",
							"english": "They ask a lot of questions about my daily routine."
						},
						{
							"portuguese": "A minha amiga está combinando um cinema para as oito da noite.",
							"english": "My friend is making plans to fo to the movies at 8:00 P.M."
						},
						{
							"portuguese": "A diretora marca a reunião para as dez da manhã.",
							"english": "The director is scheduling the meeting for 10:00 A.M."
						},
						{
							"portuguese": "Faz tempo que eu não encontro meus amigos.",
							"english": "I haven't seen my friends in a long time."
						},
						{
							"portuguese": "Vocês gostam de fazer esporte?",
							"english": "Do you like to play sports?"
						},
						{
							"portuguese": "Ele faz negócios com o Japão.",
							"english": "He does business with Japan."
						},
						{
							"portuguese": "A Estrela faz brinquedos.",
							"english": "Estrela makes toys."
						}
					]
				}
			}
		}
	}
}